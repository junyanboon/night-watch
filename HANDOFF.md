...
2. **Build the page.** CLONE `template.html`; fill only the `{{...}}` slots (masthead time + verdict, the three score numbers, the rows in the three sections: LOG, ACT I `Waiting on you` = `For = Ops Lead`, ACT II ledger). **No FIX section or score cell ever** — when ≥1 open `For = Junyan` row exists, fill the one-line `.fixnote` pointer after ACT I (count + link to The Fixer Report); omit it at zero. Keep the ACT I check-off script byte-for-byte (local ✓ strike-through, keyed by edition date). Follow `DESIGN.md`. Non-negotiables:
